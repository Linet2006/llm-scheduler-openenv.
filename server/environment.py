...
class LLMSchedulerEnvironment:
    ...
    def step(self, action: SchedulerAction) -> SchedulerObservation:
        ...
        # 5. Check Termination & Grading (STRICTLY within 0 and 1)
        ...
        if done:
            ...
            if total == 0:
                # Rule fix: must be < 1.0
                reward = 0.95 
            else:
                score = self._state.completed_requests / total
                penalty = (self._state.sla_violations / total) * 0.5
                # Rule fix: Clamp between 0.01 and 0.99
                reward = max(0.01, min(0.99, score - penalty))
            
            self._state.final_score = float(reward)

        return self._get_obs(done=done, reward=float(reward), msg=msg)
